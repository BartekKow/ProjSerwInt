...
from repositories.ipost_comment_repository import IPostRepository
from repositories.ipost_comment_repository import ICommentRepository
...
class PostRepository(IPostRepository):
    ...
    async def _parse_posts(self, posts: Iterable[dict]) -> Iterable[PostRecord]:
        return [PostRecord(userId=record.get("userId"), id=record.get("id"),
                           title=record.get('title'), body=record.get("body")) for record in posts]

class CommentRepository(ICommentRepository):
    async def get_all_comments(self) -> Iterable[CommentRecord] | None:
        all_comments = await self.get_comments_json()
        parsed_comments = await self._parse_comments(all_comments)
        return parsed_comments

    async def get_comments_json(self) -> Iterable[dict] | None:
        async with aiohttp.ClientSession() as session:
    ...
    async def _parse_comments(self, comments: Iterable[dict]) -> Iterable[CommentRecord]:
        return [CommentRecord(postId=record.get("postId"), id=record.get("id"), name=record.get('name'),
                              email=record.get("email"), body=record.get("body")) for record in comments]